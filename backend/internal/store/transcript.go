...
func (w *TranscriptWriter) Append(ctx context.Context, msg room.Message) error {
...
// given filters. Messages are returned in chronological order.
func ReadMessages(ctx context.Context, roomsDir, roomID string, opts ReadOpts) ([]room.Message, error) {
	path := filepath.Join(roomsDir, roomID+".jsonl")
	f, err := os.Open(path)
	if err != nil {
		if os.IsNotExist(err) {
...
func ReadMessagesTail(roomsDir, roomID string, offset, limit int) ([]room.Message, error) {
	if limit <= 0 {
		return []room.Message{}, nil
	}
...
func ReadMessagesFromOffset(roomsDir, roomID string, offset, limit int) ([]room.Message, error) {
	if limit <= 0 {
		return []room.Message{}, nil
	}
